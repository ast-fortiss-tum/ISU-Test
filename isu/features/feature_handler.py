...
from isu.features.models import Feature, FeatureType, CombinedFeaturesInstance, DiscreteFeature, ContinuousFeature
...
class FeatureHandler(BaseModel):
    ...
    def __getitem__(self, index) -> Optional[Feature]:
        ...
        if index in self.continuous_features:
            return self.continuous_features[index]
        return None
    ...
    def _sample_continuous_feature(self, feature: ContinuousFeature) -> float:
        ...
        res = round(sample.item(), 2)
        if res < feature.lb or res > feature.ub:
            res = np.clip(res, feature.lb, feature.ub)
        # print(f"Sampled continuous feature {feature.name}: {res}")
        return res
    ...
    def get_continuous_values_dict(self, continuous_feature_values: List[float]) -> Dict[str, Any]:
        result = {}
        for feature_name, value in zip(self.continuous_features.keys(), continuous_feature_values):
    ...
    def get_feature_values_dict(
        ...
        #print("Feature values dict:", result)
        return result
    ...
    def get_var_from_feature_value(
        ...
        if feature_type == FeatureType.CATEGORICAL or feature_type == FeatureType.ORDINAL:
