...
class CustomObjectOperatorBase(ABC):
    @staticmethod
    def _transform_problem(problem: Problem, individual: Individual, xl: Optional[np.ndarray], xu: Optional[np.ndarray]) -> Problem:
        problem = copy.deepcopy(problem)
        problem.n_var = len(individual.X)
        problem.xl = np.zeros(problem.n_var) if xl is None else np.array(xl)
        problem.xu = np.ones(problem.n_var) if xu is None else np.array(xu)
        return problem
...
class CustomObjectCrossoverBase(Crossover, CustomObjectOperatorBase, ABC):
    ...
    def _vars_crossover(
        ...
        for _ in range(self.n_offsprings):
            offspring = crossover.do(self._transform_problem(problem, parents_vars[0][0], xl, xu), parents_vars)
...
class CustomObjectMutationBase(Mutation, CustomObjectOperatorBase, ABC):
    ...
    def _vars_mutation(
        ...
        offspring_vars = mutation.do(self._transform_problem(problem, population[0], xl, xu), population)
