...
class Quality(object):
    ...
    @staticmethod
    def normalize_X_with_search_space(X, search_space):
        """
        X: (N, D_raw)
            列顺序假定为：
            [所有categorical_features, 所有continuous_features]
            对每一列按 search_space 做归一化：
            - categorical: idx / (K-1)
            - continuous: (v - lb) / (ub - lb)

        返回：
            X_norm: (N, D_norm) ∈ [0,1]^D
        """
        cat_feats = search_space.get("categorical_features", [])
        cont_feats = search_space.get("continuous_features", [])

        n_cat = len(cat_feats)
        n_cont = len(cont_feats)
        D_expected = n_cat + n_cont

        if X.shape[1] != D_expected:
            raise ValueError(
                f"X.shape[1] = {X.shape[1]}, but search_space defines {D_expected} vars "
                f"({n_cat} categorical + {n_cont} continuous)."
            )
        ...
        for feat in cat_feats:
            ...
            # 这里假定 X[:, col] 是类别 index（0 ~ K-1）
            idxs = X[:, col].astype(int)
            if K == 1:
                norm = np.zeros_like(idxs, dtype=float)
            else:
                norm = idxs / (K - 1)
            X_norm[:, col] = norm
            col += 1
    ...
    @staticmethod
    def calculate_input_hv_hitherto(result, ref_point=None):
        res = result
        hist = res.history
        if hist is None:
            return None

        n_evals, _ = res.obtain_history_hitherto(critical=False)
        ...
        # ---------------------------------------------
        # 2. Normalize X into [0,1] using problem bounds
        # ---------------------------------------------
        ...
        search_space = Quality.load_search_space("configs/isu_features_fullspace.json")
        hist_X_norm = [Quality.normalize_X_with_search_space(X, search_space) for X in hist_X_raw]

        hist_Y = [1.0 - Xn for Xn in hist_X_norm]
        n_dim = hist_Y[0].shape[1]
        ...
        hv_values = [metric_hv.do(Y) for Y in hist_Y]
        return EvaluationResult("hv_global", n_evals, hv_values)
