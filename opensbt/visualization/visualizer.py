...
def create_save_folder(problem_name: str, 
                       results_folder: str, 
                       algorithm_name: str, 
...
def optimal_individuals(res, save_folder):
    ...
    with open(
        ...
        header.append("predicts_dict")
        #header.append("params_dict")
        header.append("image_path_sim")
        #header.append("image_path_sim2real")
        ...
        for i in range(len(clean_pop)):
            ...
            elif isinstance(clean_pop.get("X")[i][0], Scenario):
                row.extend([f"%.{OUTPUT_PRECISION}f" % F_i for F_i in clean_pop.get("F")[i]])
                # row.extend([f"%s" % X_i.predicts_dict for X_i in clean_pop.get("X")[i]])
                # row.extend([f"%s" % X_i.params_dict for X_i in clean_pop.get("X")[i]])
                row.extend([f"%s" % X_i.image_path_sim for X_i in clean_pop.get("X")[i]])
                #row.extend([f"%s" % X_i.image_path_sim2real for X_i in clean_pop.get("X")[i]])
            else:
                row.extend(
    ...
    with open(save_folder + "all_testcases.csv", "w", encoding="UTF8", newline="") as f:
        ...
        for index, ind in enumerate(all_individuals):
            ...
            if isinstance(ind.get("X")[0], Utterance):
                row.extend([f"%.{OUTPUT_PRECISION}f" % X_i.ordinal_vars for X_i in ind.get("X")])
                row.extend([f"%.{OUTPUT_PRECISION}f" % X_i.question for X_i in ind.get("X")])
                row.extend([f"%.{OUTPUT_PRECISION}f" % F_i for F_i in ind.get("F")])
            elif isinstance(ind.get("X")[0], Scenario):
                row.extend([f"%.{OUTPUT_PRECISION}f" % F_i for F_i in ind.get("F")])
                # row.extend([f"%s" % X_i.predicts_dict for X_i in ind.get("X")])
                # row.extend([f"%s" % X_i.params_dict for X_i in ind.get("X")])
                row.extend([f"%s" % X_i.image_path_sim for X_i in ind.get("X")])
                #row.extend([f"%s" % X_i.image_path_sim2real for X_i in ind.get("X")])
            else:
...
def all_critical_individuals(res, save_folder):
    ...
    with open(
        ...
        for i in range(len(critical)):
            ...
            elif isinstance(critical.get("X")[i][0], Scenario):
                row.extend([f"%.{OUTPUT_PRECISION}f" % F_i for F_i in critical.get("F")[i]])
                # row.extend([f"%s" % X_i.predicts_dict for X_i in critical.get("X")[i]])
                # row.extend([f"%s" % X_i.params_dict for X_i in critical.get("X")[i]])
                row.extend([f"%s" % X_i.image_path_sim for X_i in critical.get("X")[i]])
                #row.extend([f"%s" % X_i.image_path_sim2real for X_i in critical.get("X")[i]])
            else:
                row.extend(
    ...
    for i, algo in enumerate(hist):
        with open(
            ...
            for i in range(len(algo.pop)):
                ...
                elif isinstance(algo.pop.get("X")[i][0], Scenario):
                    row.extend([f"%.{OUTPUT_PRECISION}f" % F_i for F_i in algo.pop.get("F")[i]])
                    # row.extend([f"%s" % X_i.predicts_dict for X_i in algo.pop.get("X")[i]])
                    # row.extend([f"%s" % X_i.params_dict for X_i in algo.pop.get("X")[i]])
                    row.extend([f"%s" % X_i.image_path_sim for X_i in algo.pop.get("X")[i]])
                    # row.extend([f"%s" % X_i.image_path_sim2real for X_i in algo.pop.get("X")[i]])
                else:
                    row.extend(
