...
def default_is_equal(a, b, precision=DUPLICATE_COMP_PRECISION):
    ...
    if isinstance(a, np.ndarray) and isinstance(b, np.ndarray):
        ...
        if np.issubdtype(a.dtype, np.str_) and np.issubdtype(b.dtype, np.str_):
            ...
            return eq
        #check if the elements in a are Dict
        if all(isinstance(item, Dict) for item in a) and all(isinstance(item, Dict) for item in b):
            # If both are dictionaries, compare their string representations by directly comparing the key values
            eq = all(a[0][key] == b[0][key] for key in a[0] if key in b[0])
            return eq
...
def duplicate_free(population, is_equal=default_is_equal):
    ...
    elif len(inds) > 0 and isinstance(inds[0][0], Scenario):
        inds_str = np.asarray([np.asarray([ind[0].params_dict]) for ind in inds])
        dup_free = [population[i] for i in remove_duplicates(inds_str, is_equal)]
    else:
...
def remove_duplicates(M, is_equal=default_is_equal):
    ...
    while i < size - 1:
        ...
        #print("Comparing rows:", S[i, :], "and", S[i + 1, :])
